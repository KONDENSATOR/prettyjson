...
exports.render = function render(data, options, indentation) {
  ...
  var colorize = function(input, color) {
    if (options.noColors === true) {
      return input;
    } else {
      return input[color]
    }
  ...
  if (isSerializable(data)) {
    ...
    if (data.length === 0) {
      ...
      data.forEach(function(element) {
        ...
        var line = Utils.indent(indentation) + colorize('- ', options.dashColor);
    ...
    for(var i in data) {
      ...
      key = Utils.indent(indentation) + colorize(i + ': ', options.keysColor);
